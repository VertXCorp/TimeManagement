...
public class ChangeHandler {
    ...
    private DatabaseTable timeTable;
    private DatabaseTable.ColumnWrapper uuidColumn, inColumn, outColumn, autoIncrementColumn;

    private DatabaseTable accumulatorTable;
    private DatabaseTable.ColumnWrapper totalDoingColumn, startCountColumn, firstStartColumn, accUUIDColumn;
    ...
    public ChangeHandler(String timeIn, String timeOut, String timeType) {
        ...
        BlockingQueueHandler asyncInsert = new BlockingQueueHandler(this.blockingQueue);
            ...
            if (dataFuture.isDone()) {
                try {
                    ...
                    if (data != null && data.isDoing()) {
                        ...
                        sendDebug("UPDATING TIME TO " + newRunningTotal);
                    ...
                    e.printStackTrace();
                    sendError("ERROR UPDATING ACTIVE PLAYERS");
                }
    ...
    private boolean setupTables(String timeIn, String timeOut, String timeType) {
        ...
        inColumn = new DatabaseTable.ColumnWrapper(timeIn, "BIGINT NOT NULL", "");
        outColumn = new DatabaseTable.ColumnWrapper(timeOut, "BIGINT", "");
        timeTable = new DatabaseTable("VERTX_" + timeType, autoIncrementColumn, uuidColumn, inColumn, outColumn);

        if (!createTable(timeTable))
            return false;

        accUUIDColumn = new DatabaseTable.ColumnWrapper("UUID", "VARCHAR(46) NOT NULL", "PRIMARY KEY");
        totalDoingColumn = new DatabaseTable.ColumnWrapper("TOTAL", "BIGINT", "");
        startCountColumn = new DatabaseTable.ColumnWrapper("START_NUM", "BIGINT NOT NULL", "");
        firstStartColumn = new DatabaseTable.ColumnWrapper("FIRST_START", "BIGINT NOT NULL", "");
        accumulatorTable = new DatabaseTable("VERTX_" + timeType + "_QUICK_ACCESS", accUUIDColumn, totalDoingColumn, startCountColumn, firstStartColumn);

        if (!createTable(accumulatorTable))
            return false;

        this.chatPrefix = " [" + ChatColor.BLUE + timeType + ChatColor.RESET + "]: ";

        return true;
    }

    private boolean createTable(DatabaseTable mySQLTable) {
        if (mySQLTable.create()) {
            return true;
        } else {
            sendError("Could not create table " + timeTable.getName());
            Main.disable();
            return false;
        }
    }
    ...
    private void sendDebug(String message) {
        Main.sendDebug(chatPrefix + message);
    }

    private void sendInfo(String message) {
        Main.sendInfo(chatPrefix + message);
    }

    private void sendError(String message) {
        Main.sendError(chatPrefix + message);
    }
        ...
        this.blockingQueue.add(new SequentialRunnable() {
            ...
            public boolean run() {
                try (Connection connection = SQLPool.getConnection()) {
                    sendDebug("Inserting login time for UUID " + playerUUID.toString());
                    ...
                    CompletableFuture<DataWrapper> dataFuture = CompletableFuture.supplyAsync(() -> {
                        ...
                        try (Connection connectionInside = SQLPool.getConnection()) {
                            sendDebug("Loading player from database.");
        ...
        if (!players.contains(playerUUID)) {
            CompletableFuture<DataWrapper> dataFuture = CompletableFuture.supplyAsync(() -> {
                ...
                try (Connection connection = SQLPool.getConnection()) {
                    ...
                    e.printStackTrace();
                    sendError("Could not load player. SERIOUS ERROR! SOME PLAYER DATA MAY NOT BE COLLECTED. PLEASE STOP SERVER AND FIX ISSUE");
                }
    ...
    public void timeOut(final UUID uuid) {
        ...
        data.thenCompose((dataWrap) ->
                CompletableFuture.supplyAsync(() -> {
                    if (dataWrap != null)
                        sendDebug("Setting " + uuid + " \"doing\" to false");
            ...
            public boolean run() {
                try {
                    ...
                    if (dataWrapper != null) {
                        ...
                        sendDebug("Updating player with uuid " + uuid);
